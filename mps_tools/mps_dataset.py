...
class MPSDataset:
    ...
    def h_dc_values(self):
        ...
        return np.array(
            [self.metadata.get(f"DCsteps{i:05d}", 0.0) for i in range(n_steps)]
        )
    ...
    def h_ac_values(self):
        ...
        return np.array(
            [self.metadata.get(f"ACsteps{i:05d}", 0.0) for i in range(n_steps)]
        )
    ...
    def f_ac_values(self):
        ...
        n_steps = int(
            self.metadata.get("ACFrequencySteps", self.num_ac_frequencies)
        )
        return np.array(
            [
                self.metadata.get(f"ACFrequencies{i:05d}", 0.0)
